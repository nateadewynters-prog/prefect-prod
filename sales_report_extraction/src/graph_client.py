...
from prefect import get_run_logger # Added import
...
class GraphClient:
    ...
    def _get_token(self) -> str:
        ...
        if not self._token: #
            ...
            if "access_token" in result: #
                ...
                logger.debug("🔑 Successfully acquired new MSAL access token.") # Added
            else:
    ...
    def search_emails(self, search_query: str, top: int = 100) -> List[Dict[str, Any]]:
        ...
        while endpoint: #
            ...
            if resp.status_code == 429: #
                ...
                logger.warning(f"⚠️ API Rate Limit (429) hit. Sleeping for {retry_after} seconds.") # Added
            ...
            if endpoint: 
                logger.debug("⏭️ Paginating to next set of emails via @odata.nextLink.") # Added
                time.sleep(0.5) #
            
        if not all_emails:
            logger.info(f"📭 No emails found for query: {search_query}") # Added
    ...
    def download_attachment(self, msg_id: str, expected_ext: str) -> Tuple[bytes, str]:
        ...
        for att in attachments: #
            ...
            if ext == expected_ext: #
                logger.info(f"📎 Successfully downloaded expected attachment: {att['name']}") # Added
