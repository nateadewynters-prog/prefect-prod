...
The list's display name is read from the `SHAREPOINT_RULES_LIST_NAME` environment variable (defaults to `Sales Reporting - Master List - Automation - Python`). Each rule is auto-named as `{Show Name}_{VenueName}`, uppercased with spaces replaced by underscores (e.g. `JESUS_CHRIST_SUPERSTAR_MANILA`) — this `rule_name` is the value used for the flow's `target_rule_name` parameter. Rows missing a show name (`Title`) or `VenueName` are skipped with a warning, and inactive rows (`Active` unticked or blank) are still loaded but skipped by the pipeline. The live matrix of active shows and venues is managed directly in the SharePoint List, not duplicated here.
...
- **`Title`** (shown as "Show Name" in the SharePoint UI), **`VenueName`**: Used for standard filename generation.
